...
class SourceManager:
        ...
        try:
            if os.path.exists(self.config_file):
                ...
                for source in self.sources:
                    if 'id' in source:
                        if isinstance(source['id'], str) and source['id'].isdigit():
    ...
    def remove_source(self, source_id):
        ...
        new_sources = []
        for source in self.sources:
            source_id_in_config = source.get('id')
            # 尝试将配置中的ID转换为整数进行比较
            try:
                source_id_in_config_int = int(source_id_in_config)
                if source_id_in_config_int != source_id_int:
                    new_sources.append(source)
            except:
                if source_id_in_config != source_id:
                    new_sources.append(source)
